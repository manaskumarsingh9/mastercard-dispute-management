...
# Make the Maven wrapper executable (macOS/Linux only)
chmod +x mvnw
...
Or if you set `SPRINGBOOT_URL` in your shell profile already:

```bash
PORT=5173 BASE_PATH="/" pnpm --filter @workspace/dispute-dashboard run dev
```
...
# Run in preview mode (serves built assets + runs the proxy)
...
# List disputes directly from backend
curl -s http://localhost:5000/api/disputes | python3 -m json.tool
...
# Should return: { "totalDisputes": N, "enriched": N, "averageEnrichmentPercent": N, ... }
...
```bash
# Create a dispute via the backend
...
# Run Agent 1 (Issuer Claim Summarizer) — requires GEMINI_API_KEY
curl -s -X POST http://localhost:5000/api/agents/summarize/1 | python3 -m json.tool

# Run Agent 2 (Evidence Enrichment)
...
```bash
# Runs Agents 1 → 2 → 3a → 4 in sequence
...
```bash
lsof -i :5000   # macOS — find what's using it
fuser 5000/tcp   # Linux

lsof -ti :5000 | xargs kill   # macOS — kill it
fuser -k 5000/tcp              # Linux
```
...
```bash
lsof -ti :5173 | xargs kill   # macOS
fuser -k 5173/tcp              # Linux
```
...
1. Is the backend running? `curl -s http://localhost:5000/api/disputes`
...
3. Try: `curl -s http://localhost:5173/api/disputes | python3 -m json.tool`
